...
const menuItemClassName =
  "flex cursor-pointer select-none items-center rounded-sm px-3 py-2 text-sm outline-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
          ...
          >
            {profile.menuItems.length === 0 ? (
              ...
              profile.menuItems.map((item, index) => (
                <DropdownMenu.Item key={item.id ?? `${item.href}-${item.label}-${index}`} asChild>
                  <NavbarAnchor
                    href={item.href}
